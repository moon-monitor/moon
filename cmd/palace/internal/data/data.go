...
func (d *Data) GetBizDB(teamID uint32) (gorm.DB, error) {
	...
	dsn, err := d.createDatabase(bizConf, teamID, "moon_team_")
	if err != nil {
		return nil, err
	}
	c := &config.Database{
...
func (d *Data) createDatabase(c *conf.Data_Database, teamID uint32, prefix string) (string, error) {
	if !validate.TextIsNull(c.GetDsn()) {
		return c.GetDsn(), nil
	}
	switch c.GetDriver() {
